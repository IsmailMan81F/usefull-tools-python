...
def select_pdfs():
    ...
    if paths:
        ...
        lbl_status.config(text=f"{listbox.size()} PDF(s) selected.", fg="green")

def remove_selected():
    selected = listbox.curselection()
    for i in reversed(selected):
        listbox.delete(i)
    lbl_status.config(text=f"{listbox.size()} PDF(s) selected.", fg="gray" if listbox.size() == 0 else "green")

def clear_all():
    listbox.delete(0, END)
    lbl_status.config(text="No PDFs selected.", fg="gray")
...
def convert_to_text():
    ...
    for i, pdf_path in enumerate(pdfs, start=1):
        try:
            ...
            messagebox.showerror("Error", f"Failed to extract text from {os.path.basename(pdf_path)}:\n{e}")
            return
...
Label(root, text="PDF to Text Converter", font=("Segoe UI", 16, "bold"), bg="#f4f6f8", fg="#333").pack(pady=20)
Label(root, text="Select multiple PDFs to extract text from each", bg="#f4f6f8", fg="#555").pack(pady=5)
...
Button(btn_frame, text="Browse PDFs", command=select_pdfs, bg="#3b82f6", fg="white", width=15, relief="flat", font=("Segoe UI", 10, "bold")).grid(row=0, column=0, padx=5)
...
lbl_status = Label(root, text="No PDFs selected.", bg="#f4f6f8", fg="gray")
lbl_status.pack(pady=5)

Label(root, text="Enter base text name (e.g., output):", bg="#f4f6f8", fg="#333", font=("Segoe UI", 10)).pack(pady=(20, 5))
